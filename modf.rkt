...
(define (explore e lat alloc kalloc)
  ...
  (define (step s ω)
    (match s
              ...
              (let* ((d-rands (map (lambda (æ) (atom-eval æ ρ)) e-rands))
                ...
                (let bind-loop ((xs xs) (ρ* ρ**) (d-rands d-rands))
                  (if (null? xs)
                      (let ((s* (ev e-body ρ* '() κ*))
                            (d-ret (hash-ref M κ* ⊥)))
                        (set! Ξ (hash-set Ξ κ* (set-add (hash-ref Ξ κ* (set)) ω)))
                        (set! W-inter (set-add W-inter s*))
                        (if (eq? d-ret ⊥)
                            S
                            ;(printf "call! ω ~v κ* ~v s* ~v\n" (state->statei ω) κ* (state->statei s*))
                            (begin
                              (add-transitions! s (set s*))
      ...
      ((ko d (cons (letreck a e ρ) ι) κ)
       ...
       (set (ev e ρ ι κ)))
      ((ko _ '() #f)
       ;(printf "return top-level!\n")
       ;(set! M (hash-set M κ (⊔ (hash-ref M κ ⊥) d))) could be useful to have return value of program
       (set))
      ((ko d '() κ)
       (set! W-inter (set-union W-inter (hash-ref Ξ κ)))
       (set! M (hash-set M κ (⊔ (hash-ref M κ ⊥) d)))
       (set))
      ))
  ...
  (define (inter-explore!)
    (unless (set-empty? W-inter)
      (let ((ω (set-first W-inter)))
        (set! W-inter (set-rest W-inter))
        (unless (set-member? S-inter ω)
          (set! S (set))
          (intra-explore! (set ω) ω)
          (set! S-inter (set-add S-inter ω)))
        (inter-explore!))))
               
  (define (intra-explore! W ω)
    ;(printf "intra ω ~v\n" (state->statei ω))
    (unless (set-empty? W)
      (let ((s (set-first W)))
        (if (set-member? S s)
            (intra-explore! (set-rest W) ω)
            (let* ((succs (step s ω))
